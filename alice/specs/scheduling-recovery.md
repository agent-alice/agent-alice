...
原生 interrupted 通知先同步持久化任务暂停；main 被打断时持久化全局暂停。停止树使用原生 Goal 暂停、input queue 暂停、turn interrupt、后代发现和真实终态观察；ack 本身不算停止。迟到响应不能解除暂停或继续派发。恢复与提交共用准入/输入锁，暂停代次栅栏拒绝并发暂停之后返回的恢复确认；恢复中收到配额耗尽时也持久保留待停树义务。普通退出与崩溃重启保留暂停，不自动启动新主线程。只有从未有输入/意图的空 thread 在 Codex 未保存 rollout 时可以替换。
