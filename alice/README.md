...
已激活候选的部署入口可使用 `alice-run --home "$ALICE_HOME" status`，省略最后的命令则进入 `chat`。它选择经过验证的候选解释器并保留参数边界；选择命令时校验代码和指针，不扫描业务数据库。服务启动、数据操作和版本切换仍执行各自的完整校验，因此能选择命令不代表数据库健康。

使用独立 bootstrap 的部署脚本可以调用其解释器的 `-I -m alice_codex.launcher --home "$ALICE_HOME"`。没有已激活的受检候选时会报错；首次初始化继续使用 `alice init`。旧的自定义外层入口不会自动改变，须在新产物及启动／恢复验证通过后更新。这个入口只消除最外层的一次扫描，supervisor 与服务之间的重复检查仍在 #32 跟踪。

## 当前待验范围

- 源码和 CI 工作流随候选提供；远端结果及必需检查配置需要分别核对，规范本身不提供运行能力证据。
- 当前改动后的完整 release gate、安装产物和部署／回退链路需绑定实际候选验证。
- 已有本机基础切换及原始资料归档证据，私密记录保存在仓库外；完整功能与最终组合验收仍由 #19、#20 跟踪，不能将一次基础切换视为全部迁移完成。
