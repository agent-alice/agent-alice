...
def prepare_dispatch(
    ...
    prompt = (
        ...
        "普通批次写入指定候选后，使用 Alice memory_commit_summary 工具提交其 batch_id 和候选。"
        "strategy=partitioned-v1 的批次按其 prompt 使用 memory_summary_partition_next 查询就绪节点，"
        "通过 Codex 原生子代理独立处理每个节点，一次最多并行 4 个；"
        "节点用 memory_commit_summary_partition 提交，不用 memory_commit_summary 绕过全批覆盖检查。"
        "就绪查询不是领取，不得并发重复派发同一个 node_id；已提交节点不重复执行。"
        "子任务结果不明时先核对原任务与提交回执，不能因超时换 ID 重发。"
        "分区批次只有宿主返回 complete=true、整窗口正式提交后才算完成。"
